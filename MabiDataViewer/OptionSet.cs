...
namespace MabiDataViewer
{
    internal class OptionSet
    {
        ...
        public String GetOptionSetSucceseRate(double OptionSetSuccessRate, double powder_rates, int level_rates, string OptionSetLevel, string OptionSetIsAlwaysSuccess, CheckBox OptionSetDayBonus, CheckBox OptionSetHelperBonus)
        {
            ...
            OptionSetSuccessRate = OptionSetSuccessRate >= 90 ? 90 : OptionSetSuccessRate;
            string Result = OptionSetLevel != "0" && OptionSetIsAlwaysSuccess != "true" ? $"((200 - 25) / 350 + 1 + {powder_rates}{(OptionSetDayBonus.Checked ? " + 0.1" : "")}) * {level_rates}{(OptionSetHelperBonus.Checked ? " + 10" : "")} = {OptionSetSuccessRate}%" : "100%";
            return Result;
        }
